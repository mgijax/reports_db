...
db.sql('''
        select convert(int,a1.accID) as homologeneID,
               o._Organism_key, o.commonName,
               m._Marker_key, m.symbol, m.name,
               a2.accID as entrezgeneID
        into #temp1
        from ACC_Accession a1, MRK_Cluster c, MRK_ClusterMember cm,
             MRK_Marker m, MGI_Organism o, ACC_Accession a2
        where a1._LogicalDB_key = 81 and
              ...
              cm._Marker_key = a2._Object_key and
...
#
results = db.sql('''
        select t._Marker_key, lc.chromosome, lc.cytogeneticOffset,
               str(lc.offset,10,2) as offset, lc.genomicChromosome,
               str(lc.startCoordinate,10,0) as startCoordinate,
               str(lc.endCoordinate,10,0) as endCoordinate,
               lc.strand
        from #temp1 t, MRK_Location_Cache lc
        where t._Marker_key = lc._Marker_key and
              lc._Organism_key in (1,2)
        union
        select t._Marker_key, m.chromosome, m.cytogeneticOffset,
               NULL, NULL, NULL, NULL, NULL
        from #temp1 t, MRK_Marker m
...
#
results = db.sql('''
        select t._Marker_key, s.synonym
        from #temp1 t, MGI_Synonym s
        where t._Marker_key = s._Object_key and
              s._MGIType_key = 2
        order by t._Marker_key, s.synonym
        ''','auto')
...
#
results = db.sql('''
        select homologeneID, _Organism_key, commonName, _Marker_key,
               symbol, name, entrezgeneID
        from #temp1 
        order by homologeneID, _Organism_key, symbol
        ''','auto')
