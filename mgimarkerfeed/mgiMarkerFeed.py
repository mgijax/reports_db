...
# sc - tested this in 3.7 python interpreter
def strip_newline(s):
...
def vocabs():
    ...
    results = db.sql('''
            ...
            ''', 'auto', execute = 1)
    for r in results:
            fp.write(repr(r['_Marker_Type_key']) + TAB + \
                     r['name'] + TAB + \
                     str(r['cdate']) + TAB + \
                     str(r['mdate']) + CRT)
    fp.close()
    ...
    results = db.sql('''
            ...
            ''', 'auto', execute = 1)
    for r in results:
            fp.write(repr(r['_Organism_key']) + TAB + \
                     r['commonName'] + TAB + \
                     r['latinName'] + TAB + \
                     str(r['cdate']) + TAB + \
                     str(r['mdate']) + CRT)
    ...
    for r in results:
            fp.write(r['accID'] + TAB + \
                     r['LogicalDB'] + TAB + \
                     repr(r['_Object_key']) + TAB + \
                     repr(r['preferred']) + TAB + \
                     str(r['cdate']) + TAB + \
                     str(r['mdate']) + CRT)
    fp.close()
    ...
    for r in results:
        fp.write(repr(r['_AncestorObject_key']) + TAB + \
                 repr(r['_DescendentObject_key']) + TAB + \
                 repr(r['_AncestorLabel_key']) + TAB + \
                 repr(r['_DescendentLabel_key']) + CRT)
...
def markers():
    ...
    for r in results:
            fp.write(repr(r['_Marker_key']) + TAB + \
                     repr(r['_Organism_key']) + TAB + \
                     repr(r['_Marker_Type_key']) + TAB + \
...
def alleles():
    ...
    results = db.sql('''
            ...
            ''', 'auto', execute = 1)
    for r in results:
            fp.write(repr(r['_CellLine_key']) + TAB + \
                     r['cellLine'] + TAB + \
                     repr(r['_CellLine_Type_key']) + TAB + \
                     repr(r['_Strain_key']) + TAB + \
                     repr(r['_Derivation_key']) + TAB + \
                     repr(r['isMutant']) + TAB + \
                     str(r['cdate']) + TAB + \
                     str(r['mdate']) + CRT)
    fp.close()
    ...
    results = db.sql('''
        select _Derivation_key, name, 
            ...
            ''', 'auto', execute = 1)
    for r in results:
            fp.write(repr(r['_Derivation_key']) + TAB + \
                     r['name'] + TAB + \
                     repr(r['_Vector_key']) + TAB + \
                     repr(r['_VectorType_key']) + TAB + \
                     repr(r['_ParentCellLine_key']) + TAB + \
                     repr(r['_DerivationType_key']) + TAB + \
                     repr(r['_Creator_key']) + TAB + \
                     repr(r['_Refs_key']) + TAB + \
                     str(r['cdate']) + TAB + \
                     str(r['mdate']) + CRT)
    fp.close()
    ...
    for r in results:
            fp.write(repr(r['_Allele_key']) + TAB + \
                     repr(r['_Marker_key']) + TAB + \
                     repr(r['_Mode_key']) + TAB + \
                     repr(r['_Allele_Type_key']) + TAB + \
                     repr(r['_Transmission_key']) + TAB + \
                     mgi_status(r['status']) + TAB + \
                     repr(r['_Strain_key']) + TAB + \
                     repr(r['_Collection_key']) + TAB + \
                     repr(r['isWildType']) + TAB + \
                     repr(r['isExtinct']) + TAB + \
                     repr(r['isMixed']) + TAB + \
                     strip_newline(r['symbol']) + TAB + \
                     strip_newline(r['name']) + TAB + \
                     str(r['cdate']) + TAB + \
    ...
    for r in results:
            fp.write(repr(r['_Assoc_key']) + TAB + \
                     repr(r['_Allele_key']) + TAB + \
                     repr(r['_MutantCellLine_key']) + TAB + \
                     str(r['cdate']) + TAB + \
                     str(r['mdate']) + CRT)
    fp.close()
...
def strains():
    ...
    for r in results:
            fp.write(repr(r['_Strain_key']) + TAB + \
                     repr(r['_Species_key']) + TAB + \
...
#         and m.synonym not like 'nm[0-9]%' ', 'auto')
    ...
    for r in results:
            fp.write(repr(r['_Synonym_key']) + TAB + \
                     repr(r['_Object_key']) + TAB + \
                     repr(r['private']) + TAB + \
...
def genotypes():
    ...
    for r in results:
        ...
        note = str.replace(str.strip(r['note']), '\n', ' ')

        fp.write(repr(r['_Genotype_key']) + TAB + \
                r['strain'] + TAB + \
                repr(r['isConditional']) + TAB + \
                repr(r['_ExistsAs_key']) + TAB + \
                note + TAB + \
                str(r['cdate']) + TAB + \
                str(r['mdate']) + CRT)
    fp.close()
    ...
    for r in results:
        ...
        fp.write(repr(r['_Annot_key']) + TAB + \
                 repr(r['_AnnotType_key']) + TAB + \
                 repr(r['_Term_key']) + TAB + \
                 repr(r['_Genotype_key']) + TAB + \
                str.strip(mgi_utils.prvalue(r['qualifier'])) + TAB + \
                str(category) + TAB + \
                str(r['cdate']) + TAB + \
                str(r['mdate']) + CRT)
    ...
    for r in results:
        fp.write(repr(r['_Genotype_key']) + TAB + \
                 repr(r['headerTerm']) + TAB + \
                 repr(r['sequenceNum']) + TAB + \
                str(r['cdate']) + TAB + \
                str(r['mdate']) + CRT)
    ...
    for r in results:
            fp.write(repr(r['_AllelePair_key']) + TAB + \
                     repr(r['_Allele_key_1']) + TAB + \
                     mgi_utils.prvalue(r['_Allele_key_2']) + TAB + \
                     repr(r['_Marker_key']) + TAB + \
                     repr(r['_Genotype_key']) + TAB + \
                     repr(r['_PairState_key']) + TAB + \
                     repr(r['sequenceNum']) + TAB + \
                     str(r['cdate']) + TAB + \
                     str(r['mdate']) + CRT)
    fp.close()
...
def references():
    ...
    db.sql('''
           ...
           where a._AnnotType_key in (1022)
           and a._Annot_key = e._Annot_key
           ''', None)
    ...
    for r in results:
            fp.write(repr(r['_Object_key']) + TAB + \
                     repr(r['_Refs_key']) + TAB + \
                     r['assocType'] + TAB + \
                     str(r['cdate']) + TAB + \
                     str(r['mdate']) + CRT)
...
def mgi_relationship():
    ...
    for r in results:
        fp.write(repr(r['_relationship_key']) + TAB + \
                 repr(r['_category_key']) + TAB + \
                 repr(r['_object_key_1']) + TAB + \
                 repr(r['_object_key_2']) + TAB + \
                 repr(r['_relationshipterm_key']) + TAB + \
                 repr(r['_qualifier_key']) + TAB + \
                 repr(r['_evidence_key']) + TAB + \
                 repr(r['_refs_key']) + TAB + \
                 str(r['cdate']) + TAB + \
                 str(r['mdate']) + CRT)
    ...
    for r in results:
        fp.write(repr(r['_category_key']) + TAB + \
                 r['name'] + TAB + \
                 repr(r['_relationshipvocab_key']) + TAB + \
                 repr(r['_relationshipdag_key']) + TAB + \
                 repr(r['_mgitype_key_1']) + TAB + \
                 repr(r['_mgitype_key_2']) + TAB + \
                 repr(r['_qualifiervocab_key']) + TAB + \
                 repr(r['_evidencevocab_key']) + TAB + \
                 str(r['cdate']) + TAB + \
                 str(r['mdate']) + CRT)
    ...
    for r in results:
        fp.write(repr(r['_relationshipproperty_key']) + TAB + \
                 repr(r['_relationship_key']) + TAB + \
                 repr(r['_propertyname_key']) + TAB + \
                 r['value'] + TAB + \
                 repr(r['sequenceNum']) + TAB + \
                 str(r['cdate']) + TAB + \
                 str(r['mdate']) + CRT)
    ...
    results = db.sql('''
            ...
            ''', 'auto', execute = 1)
    for r in results:
            fp.write(repr(r['_Vocab_key']) + TAB + \
                     repr(r['_Term_key']) + TAB + \
                     r['name'] + TAB + \
                     r['term'] + TAB + \
                     str(r['cdate']) + TAB + \
