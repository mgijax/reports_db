...
fp.write('#\n')
fp.write('#  column 1: mutant cell line ID\n')
...
db.sql('select a._Allele_key, a._Marker_key, a.symbol, a.name, ' + \
	'mutantID = aa.accID, ldb = ldb.name, markerSymbol = m.symbol ' + \
	'into #komp ' + \
	'from ALL_Allele a, ALL_CellLine c, ACC_Accession aa, ACC_LogicalDB ldb, MRK_Marker m ' + \
        'where a._MutantESCellLine_key = c._CellLine_key ' + \
        'and c._CellLine_key = aa._Object_key ' + \
        'and aa._MGIType_key = 28 ' + \
        'and aa._LogicalDB_key in (108,109) ' + \
...
# process results
#
results = db.sql('select * from #komp order by mutantID', 'auto')

for r in results:

    fp.write(r['mutantID'] + TAB + \
	     r['ldb'] + TAB + \
